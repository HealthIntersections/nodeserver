...
class Library {
  ...
  async processSource(source, packageManager, mode) {
    ...
    switch (type) {
      ...
      case 'loinc-sqlite-v0':
        await this.loadSqliteV0(details, isDefault, mode, { idPrefix: 'sqlite-v0' });
        break;
      case 'sqlite-v0':
        await this.loadSqliteV0(details, isDefault, mode, { idPrefix: 'sqlite-v0' });
        break;
      ...
      case 'snomed':
        throw new Error(
          `Legacy source type '${type}' is not supported in this branch. ` +
          `Use sqlite-v0:<db-file> sources generated by sqlite-v2 importers.`
        );
  ...
  async loadSqliteV0(details, isDefault, mode, options = {}) {
    ...
    const idPrefix = options.idPrefix || 'sqlite-v0';
