...
class RxNormSqliteV0Importer {
  ...
  async run() {
    ...
    try {
      ...
      this.log(
        ...
        `RXNATOMARCHIVE=${bool(files.rxnatomarchive)}`
      );

      await this.importConcepts(files.rxnconso);
      await this.importArchivedConcepts(files.rxnatomarchive);
  ...
  async importArchivedConcepts(archiveFile) {
    if (!archiveFile) {
      this.log('RXNATOMARCHIVE.RRF not found — skipping archived concept import');
      return;
    }
    this.log('Importing archived concepts from RXNATOMARCHIVE.RRF...');

    // RXNATOMARCHIVE columns: RXAUI[0] AUI[1] STR[2] ARCHIVE_TIMESTAMP[3]
    // CREATED_TIMESTAMP[4] UPDATED_TIMESTAMP[5] CODE[6] IS_BRAND[7] LAT[8]
    // LAST_RELEASED[9] SAUI[10] VSAB[11] RXCUI[12] SAB[13] TTY[14] MERGED_TO_RXCUI[15]

    // First pass: collect best display per RXCUI not already in concept table
    const archived = new Map();
    let scanned = 0;

    for await (const cols of readRrf(archiveFile)) {
      ...
      const rxcui = cols[12];
      if (!rxcui || this.conceptIdByCode.has(rxcui)) continue;

      const str = (cols[2] || '').trim();
      const tty = cols[14] || null;
      const rank = ttyRank(tty);

      const existing = archived.get(rxcui);
      if (!existing || rank < existing.rank) {
        archived.set(rxcui, { display: str || rxcui, tty, rank });
      }
    }

    if (archived.size === 0) {
      this.log(`  scanned ${scanned.toLocaleString()} rows, no new archived concepts`);
      return;
    }
    ...
    for (const [rxcui, info] of archived.entries()) {
      const conceptId = this.nextConceptId++;
      this.conceptIdByCode.set(rxcui, conceptId);

      conceptRows.push([conceptId, this.csId, rxcui, 0, info.display, null]);

      if (info.tty) {
        ttyLiteralRows.push([
          EDGE_SET_INFERRED, conceptId, this.ttyPropertyId, 0, 0,
          info.tty, info.tty, null, null
        ]);
      }
      imported += 1;
    ...
    this.stats.concepts += imported;
    this.stats.literals += ttyLiteralRows.length;
    this.log(`Archived concept import complete: ${imported.toLocaleString()} concepts from ${scanned.toLocaleString()} archive rows`);
  }
...
function scanDirectoryForRrf(dir, files) {
  ...
  for (const entry of entries) {
    ...
    else if (name === 'RXNATOMARCHIVE.RRF') files.rxnatomarchive = fullPath;
  }
}
