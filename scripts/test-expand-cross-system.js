...
async function main() {
  ...
  try {
    ...
    try { await httpPost(`http://localhost:${PORT}/debug/perf-counters/enable`); } catch (_) {}
    ...
    for (let ti = 0; ti < testList.length; ti++) {
      ...
      let baseMs = 0, baseCodes = null, baseStatus = null, baseSkipped = false;
      ...
      if (cmpBase.match === false && test.drainCount) {
        ...
        try { drainOpt = await postJson(BASE_URL + '/ValueSet/$expand', drainBody, 120000); } catch (_) {}
        try { drainBase = await postJson(REF_URL + '/ValueSet/$expand', drainBody, 120000); } catch (_) {}
